...
class Raft:
    ...
    def __init__(self, address, peers, election_timeout_min=5, election_timeout_max=10):
        base_cache_dir = os.getenv("XDG_CACHE_HOME") or os.path.join(
        ...
        self.election_timer = RestartableTimer(
            random.uniform(election_timeout_min, election_timeout_max),
            self.election_timeout,
        )
        self.heartbeat_timer = RestartableTimer(
            election_timeout_min / 2, self.heartbeat_timeout
        )
        self.commit_index = 0
        self.last_applied = 0
    ...
    def election_timeout(self):
        ...
        log.info(
            "election_result",
            votes_received=self.term.votes,
            majority=majority,
            term=self.term.current_term,
        )
    ...
    def append_entries_rpc(
        ...
        if term < self.term.current_term:
            return {"term": self.term.current_term, "success": False}
        ...
        return {"term": self.term.current_term, "success": True}
    ...
    def request_vote_rpc(
        ...
        if term < self.term.current_term:
            return {"term": self.term.current_term, "vote_granted": False}

        if (
            self.term.voted_for is None or self.term.voted_for == candidate_id
        ) and last_log_index >= len(self.log):
